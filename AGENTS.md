...
work machine — using Codex on all of them. Codex conversations are stored per-machine and **do not sync**. Git is
...
`.Codex/settings.json` (committed, so every machine gets it) wires two hooks:
...
commit it, never paste it into a Codex chat).
