...
const SEED_VERSION = 28; // bump this each time I add/update words
...
function Study({ cards, onResult, goAdd }) {
  ...
  const batches = useMemo(() => {
    ...
    return Array.from(map.keys()).sort((a, b) => sectionRank(a) - sectionRank(b)).map((sec) => {
      const group = map.get(sec);
      let seen = 0, correct = 0;
      group.forEach((c) => { seen += c.seen || 0; correct += c.correct || 0; });
      return { name: sec, cards: group, rate: seen ? correct / seen : null };
    });
  }, [cards]);
...
/* ── Japanese text-to-speech (browser built-in) ── */
...
function stopJa() { if (TTS_OK) { try { window.speechSynthesis.cancel(); } catch (e) {} } }
